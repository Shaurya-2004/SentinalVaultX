...
#             os._exit(0)  # Close system immediately

#         time.sleep(2)  # Check approval status every 2 seconds
...
#                 alerts.send_email(face_id, "das394751@gmail.com")
#                 alerts.send_sms(face_id, "+917081239249")
#                 print(f"[ALERT] Unauthorized alert sent for {face_id}")
                
#                 approval_thread = threading.Thread(target=wait_for_approval, args=(face_id, embedding, frame, bbox))
#                 approval_thread.daemon = True
#                 approval_thread.start()
            
#             else:
#                 cv2.putText(frame, f"Identity: {identity}", (30, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
